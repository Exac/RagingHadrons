<style type="text/css" scoped="scoped">

	.game_img{
		position: relative;
	}
	.gamename{
		font-size:70px;
		color:#fff;
		display:inline-block;
		margin:0px auto 0px auto;
		background-image: url(data:image/png;base64,);
		position: absolute;
		bottom:0px;
	}
	.bui{
		padding: 0px;
	}
</style>
<div id="maingrid">
	<section id="jumbotron">
		<img src="/upload/game/dota2/clan_pic_2.jpg" alt="Raging Hadrons Clan"/>
		<div class="excerpt">
			<p>Check out our Professional DOTA 2 team, currently playing in the RDL!</p>
		</div>
		<h1>Dota 2</h1>
		</section>

		<div class="row">
			<section class="half ll">
				<a href="/upload/game/dota2/clan_pic.jpg" target="_blank"><img alt="" src="/upload/game/dota2/clan_pic.jpg" style="height:100%;" /></a>
			</section>

			<section class="half ll">
				<h2>Mumble Server</h2>
				<p>IP: <q class="code">72.5.102.12</q><br>Port: <q class="code">10640</q><br>Password: available in the RD2L guild channel</p>
			</section>
		</div>
		<div class="row">
			<section class="full ll">
				<h2>Team BIO</h2>
				<p>Raging Hadrons Gaming first saw the light of day in the summer of 2011, and has since then become one of the most successful teams in the history of Dota 2. With victories at The International 2, the World Cyber Games, and G-League, it is fair to say that Invictus Gaming is a dominant force in the international gaming scene. This August, Team iG will be heading to Seattle with only one goal in mind: to return with the glory and the championship--their names once again emblazoned on the Aegis of Champions. </p>
			</section>
		</div>
	</div>
</section>